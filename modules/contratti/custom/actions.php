...
switch (post('op')) {
    case 'add':
        ...
        if (isset($post['idanagrafica'])) {
            ...
            //Inserisco solo il tipo attività 'VIS-ABB' che è la visita in abbonamento
            //$rsi = $dbo->fetchArray('SELECT * FROM in_tipiintervento WHERE (costo_orario!=0 OR costo_km!=0 OR costo_diritto_chiamata!=0)');
            $rsi = $dbo->fetchArray('SELECT * FROM in_tipiintervento WHERE idtipointervento="VIS-ABB"');
    ...
    case 'update':
        if (isset($post['id_record'])) {
            ...
            $rs = $dbo->fetchArray('SELECT * FROM co_contratti_tipiintervento WHERE idcontratto='.prepare($id_record).' AND idtipointervento='.prepare($idtipointervento));
                ...
                // Se non esiste il record lo inserisco (tolto il controllo se valori 0)
                //if ((sizeof($rs) == 0) ) {
                    // inserisco ...
            ...
            // stravolto completamente la gestione dei costi dei contratti
            // nei contratti è possibile gestire solo il tipo intervento di Visita in Abbonamento
            // senza nessun
            // Aggiorno il nuovo valore ...
