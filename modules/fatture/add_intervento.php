...
$rs = $dbo->fetchArray('SELECT in_interventi.id, CONCAT(\'Rapportino intervento n.\', num_rapp, \' del \', DATE_FORMAT(IFNULL((SELECT MIN(orario_inizio) FROM in_interventi_tecnici WHERE in_interventi_tecnici.idintervento=in_interventi.id), data_richiesta), \'%d/%m/%Y\')) AS descrizione, IF(idclientefinale='.prepare($idanagrafica).', \'Interventi conto terzi\', \'Interventi diretti\')  AS `optgroup`FROM in_interventi WHERE (idanagrafica='.prepare($idanagrafica).' OR idclientefinale='.prepare($idanagrafica).') AND (idstatointervento=\'DA_FAT\' OR idstatointervento=\'DA_FAT_DET\' OR idstatointervento=\'DA_FAT_P\') AND in_interventi.id NOT IN (SELECT idintervento FROM co_righe_documenti WHERE idintervento IS NOT NULL)');
